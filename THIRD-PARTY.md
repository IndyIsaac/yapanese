...
Yapanese is MIT licensed. It builds on the following work.
...
These are separate programs Yapanese runs. They are not redistributed as part of
...
| [ffmpeg](https://ffmpeg.org/) | LGPL-2.1+ / GPL-2+ | Optional. Only used by the `yap` fallback path; Yapanese's own capture path does not need it |

## Origin

Yapanese began while porting [finnvoor/yap](https://github.com/finnvoor/yap)
...
Yapanese is a separate program with a different purpose, and it no longer
